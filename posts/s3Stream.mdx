...
Amazon S3 is a cloud object storage service. Each S3 bucket's size scales as needed, bespoke permissions are easy to setup, and CRUD opperations can be performed
...
of running out of space on the EC2 container and affecting the functionality of my clients site so I decided to store uploaded files in public/private S3 containers as needed.
...
a brand new object storage container in a few minutes. Make a publick accessable bucket and a privately accessable bucket. At this point you should have two buckets but
...
Click Net: Tags, Next: Review, and add a name and description then click Create policy. Your policy is now create but you still need to attach the policy to a user.
Click over to Users, select your user, and add permission. Search for the name of your policy and add add it to the user. At this point you should now have two buckets
...
the frontend of their site any affected pages are recompiled useing Next.js's [On-Demand Incremental Static Regeneration](https://nextjs.org/docs/basic-features/data-fetching/incremental-static-regeneration#on-demand-revalidation-beta).
As a result a non-technical user can redesign and republish most of the site on the fly with no action needed from their webmater(me).
