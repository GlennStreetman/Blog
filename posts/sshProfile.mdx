...
This article assumes you are using Bash.
...
you can create one with the command below.  
-C allows you to add a description.  
Make sure to enter a passphrase for your key.
...
Using SSH agent you can enter your ssh password once per window then open as many connections as you want without having to re-enter the password.
...
<CodeBlock
    ...
    if [ -n "\$1" ]; then
        runKey=\$1p
        IDKEY=\$(ls ~/.ssh/ -1 | grep -v pub | sed -n 2p)
        eval \$(ssh-agent)
        ssh-add ~/.ssh/\${IDKEY}
    else
        echo "Append key number to ssha command to add key to ssh agent"
