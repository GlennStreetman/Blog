...
export const Meta = {
    id: "buildScript",
    title: "Next.js Blogs/Wiki posts using front-matter style metadata and predefined views/layouts",
    date: "2022-01-18",
    type: "notes",
...
The Next.js docs do offer somewhat vague hint on how to add Grey Matter style hints if you want to use MDX:
...
After you gather up all of your metadata what do you do with it?. Along the way i ran into some cryptic debugging messages genered by
...
## Use dynamic routes to wrap each post in a consitent layout.
...
Note that the code exmple below also references a function that sorts my project metadata.
...
Now each /posts/\*.mdx route has the same layout/view, can access its own metadata, find the metadata for any related projects, and using javascript components. Creating and editing
posts should be a simple task going forward.
