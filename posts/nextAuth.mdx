...
[https://next-auth.js.org/](NextAuth.js) is an open source [0Auth](https://oauth.net/) solution for [Next.js](https://nextjs.org/) applications.
...
Express app. If you are not familier with NextAuth you should go through the [Next Auth Tutorial](https://next-auth.js.org/tutorials) first. Make sure to setup a database [nextAdapter](https://next-auth.js.org/adapters/overview) as
...
divided up using subdomains this will violate the client side CORS:[Same-origin policy](https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy),
...
## Checking shared login status with Nextjs app
    ...
    server is persisting sessions using a database [nextAdapter](https://next-auth.js.org/adapters/overview). All you need to do is install NextAuth as a dependancy of each Next application
...
## Create a logout route so non-Nextjs applications can logout without redirecting the client to the NextAuth application.
