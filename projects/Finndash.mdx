...
## React:
    ...
    make use of asynchronous API calls. After API data was retrieved a series of state updated would need to fire off to render and cache the data. A library to help manage this process would simplify 
...
## Redux and Redux Tool Kit:
    ...
    API calls, additional data can be cached to MongoDB. The data stored on the client-side then needs to be converted into a standard format using derived state. 
...
## Frontend Tests
    Frontend tests are isolated from any HTTP requests using a mock HTTP server. By mocking HTTP responses, tests can be performed against consistent HTTP return data 
